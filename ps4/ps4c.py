    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        for letter in string.ascii_letters:
            if letter in VOWELS_LOWER:
                index_org = VOWELS_LOWER.find(letter)
                transpose_dictionary.update({letter: vowels_permutation[index_org]})
            elif letter in VOWELS_UPPER:
                index_org = VOWELS_UPPER.find(letter)
                transpose_dictionary.update({letter: vowels_permutation_upper[index_org]})
            else:
                transpose_dictionary.update({letter: letter})
...
if __name__ == '__main__':
    ...
    #TESTCASE2
    message = SubMessage("This program is created by Hayk Stepanyan")
    permutation = "auioe"
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(), "Permutation:", permutation)
    print("Expected encryption:", "This program is cruatud by Hayk Stupanyan")
    print("Actual encryption:", message.apply_transpose(enc_dict))
    enc_message = EncryptedSubMessage(message.apply_transpose(enc_dict))
    print("Decrypted message:", enc_message.decrypt_message())
    print()
    ...
    #TESTCASE3
    message = SubMessage("Armenia")
    permutation = "uioea"
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(), "Permutation:", permutation)
    print("Expected encryption:", "Urminou")
    print("Actual encryption:", message.apply_transpose(enc_dict))
    enc_message = EncryptedSubMessage(message.apply_transpose(enc_dict))
    print("Decrypted message:", enc_message.decrypt_message())
