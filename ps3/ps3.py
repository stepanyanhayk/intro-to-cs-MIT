...
# Time spent    : 3 days, finished on May 30
...
def update_hand(hand, word):
    ...
    hand_copy = hand.copy()
    for letter in word.lower():
        for character in hand_copy.keys():
            if letter == character:
                hand_copy[character] -= 1
    hand = hand_copy.copy()
...
def is_valid_word(word, hand, word_list):
    ...
    def check_validity(hand, word):
        hand_copy = hand.copy()
        for letter in word.lower():
            if letter not in hand.keys() or hand_copy[letter] == 0:
                return False
            hand_copy[letter] -= 1
        return True
    ...
    if "*" in word:
        for letter in VOWELS:
            word_copy = list(word[:])
            word_copy[wildcard_index] = letter
            if "".join(word_copy) in word_list:
                match += 1
        if match == 0:
            return False
        return check_validity(hand, word)
    else:
    ...
    while calculate_handlen(hand) != 0:
        ...
        if word == "!!":
            ...
            if is_valid_word(word, hand, word_list):
                ...
                print(word, "earned", get_word_score(word, HAND_SIZE), "points. Total:", total_score, "points")
                print("----------")
            else:
    ...
    * Returns the total score for the series of hands
    ...
    while hand_count != 0:
        ...
        if substitute == "yes":
            substitute_letter = input("Which letter would you like to replace? ")
            substitute_hand(hand, substitute_letter)
        the_total_score += play_hand(hand, word_list)
        hand_count -= 1
    print("Total score:", the_total_score)
#
